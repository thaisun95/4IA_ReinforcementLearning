...
def expected_sarsa(
    ...
    """
    Expected SARSA (harmonized for variable action spaces).
    Compatible with envs where valid actions depend on state.
    """
    ...
    for episode in range(episodes):
        ...
        while not env.is_terminal(state_obj):
            ...
            # Epsilon-greedy action selection on valid actions
            ...
            if not done:
                ...
                q_next = [Q[next_state, env.action_space.index(a)] for a in next_valid_actions]
                max_q = max(q_next)
                n_max = sum([1 for v in q_next if v == max_q])
                expected_q = 0
                for a, q_val in zip(next_valid_actions, q_next):
                    if q_val == max_q:
                        prob = (1 - epsilon) / n_max + epsilon / len(next_valid_actions)
                    else:
                        prob = epsilon / len(next_valid_actions)
...
def evaluate_policy(env, policy, episodes=100):
    ...
    Compatible with environments à actions valides (policy[state_idx] = action or -1).
    """
    total_rewards = []
    total_lengths = []
    ...
    for _ in range(episodes):
        ...
        while not env.is_terminal(state_obj):
            action = policy[state]
            if action == -1:  # Terminal state (shouldn't happen here, but just in case)
                break

            # Vérifie si action est valide (important si tu n'as pas convergé partout)
